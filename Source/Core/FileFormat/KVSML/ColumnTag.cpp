...
bool ColumnTag::read( const kvs::XMLElement::SuperClass* element )
{
    ...
    // label="xxx"
    const auto label = kvs::XMLElement::AttributeValue( element, "label" );
    if ( label != "" ) { this->setLabel( label ); }

    // min_value="xxx"
    const auto min_value = kvs::XMLElement::AttributeValue( element, "min_value" );
    if ( min_value != "" ) { this->setMinValue( atof( min_value.c_str() ) ); }

    // max_value="xxx"
    const auto max_value = kvs::XMLElement::AttributeValue( element, "max_value" );
    if ( max_value != "" ) { this->setMaxValue( atof( max_value.c_str() ) ); }

    // min_range="xxx"
    const auto min_range = kvs::XMLElement::AttributeValue( element, "min_range" );
    if ( min_range != "" ) { this->setMinRange( atof( min_range.c_str() ) ); }

    // max_range="xxx"
    const auto max_range = kvs::XMLElement::AttributeValue( element, "max_range" );
...
bool ColumnTag::write( kvs::XMLNode::SuperClass* parent )
{
    ...
    if ( m_has_label ) { element.setAttribute( "label", m_label ); }
    if ( m_has_min_value ) { element.setAttribute( "min_value", m_min_value ); }
    if ( m_has_max_value ) { element.setAttribute( "max_value", m_max_value ); }
    if ( m_has_min_range ) { element.setAttribute( "min_range", m_min_range ); }
    if ( m_has_max_range ) { element.setAttribute( "max_range", m_max_range ); }
