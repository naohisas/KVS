...
ValueAxis::ValueAxis( Align align ):
    m_visible( true ),
    m_align( align ),
    m_width( 2 ),
    ...
    m_label( "" ),
    m_label_font( kvs::Font::Sans, kvs::Font::Bold, 20.0f ),
    m_label_offset( 5 ),
    m_tick_mark_visible( true ),
    m_tick_mark_width( 2 ),
    m_tick_mark_length( 5 ),
    m_tick_label_visible( true ),
    m_tick_label_font( kvs::Font::Sans, kvs::Font::Regular, 18.0f ),
...
ValueAxis::ValueAxis( const ValueAxis& axis ):
...
int ValueAxis::drawTickLabelsOnTop( kvs::Painter& painter )
{
    ...
    const float x0 = m_rect.x0();
    const float x1 = m_rect.x1();
    const float y0 = m_rect.y0();
