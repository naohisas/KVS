#include "ColoredMarchingCubes.h"
 ...
 *  @param  volume_list [in] pointer to the volume object list
 ...
 */
/*==========================================================================*/
ColoredMarchingCubes::ColoredMarchingCubes(
 ...
 */
/*===========================================================================*/
ColoredMarchingCubes::SuperClass* ColoredMarchingCubes::exec( const kvs::ObjectBase* object )
{
    ...
    const auto* volume_list = kvs::StructuredVolumeObjectList::DownCast( object );
    if ( !volume_list )
    {
        kvsMessageError("Input object is not structured volume object list.");
        return nullptr;
    }

    if ( volume_list->objects().size() < 2 )
    {
        kvsMessageError("Input object is not multivariate structured volume object.");
        return nullptr;
    }

    const Volume* volume0 = &volume_list->objects()[0]; // add
    const Volume* volume1 = &volume_list->objects()[1]; // add
    if ( volume0->veclen() != 1 || volume1->veclen() != 1 )
    {
    ...
    // add {
    m_values = volume1->values();
    m_min_value = volume1->minValue();
    m_max_value = volume1->maxValue();
    // }

    this->mapping( volume0 );
 ...
 */
/*==========================================================================*/
void ColoredMarchingCubes::mapping( const Volume* volume )
{
...
/*==========================================================================*/
template <typename T>
void ColoredMarchingCubes::extract_surfaces( const Volume* volume )
{
    if ( m_duplication )
    {
        if ( m_values.size() > 0 )
            this->extract_colored_surfaces_with_duplication<T>( volume ); // add
        else
    ...
    {
        if ( m_values.size() > 0 )
            this->extract_colored_surfaces_without_duplication<T>( volume ); // add
        else
...
/*==========================================================================*/
template <typename T>
void ColoredMarchingCubes::extract_surfaces_with_duplication( const Volume* volume )
{
    ...
    // Extract surfaces.
    auto Edge = kvs::MarchingCubesTable::TriangleID;
    auto Vert = kvs::MarchingCubesTable::VertexID;
    size_t index = 0;
...
/*==========================================================================*/
template <typename T>
void ColoredMarchingCubes::extract_surfaces_without_duplication( const Volume* volume )
{
...
/*==========================================================================*/
template <typename T>
size_t ColoredMarchingCubes::calculate_table_index( const size_t* local_index ) const
{
...
/*==========================================================================*/
template <typename T>
const kvs::Vec3 ColoredMarchingCubes::interpolate_vertex(
...
/*==========================================================================*/
template <typename T>
void ColoredMarchingCubes::calculate_isopoints(
...
/*==========================================================================*/
template <typename T>
void ColoredMarchingCubes::connect_isopoints(
    kvs::UInt32*& vertex_map,
    std::vector<kvs::UInt32>& connections )
{
    const auto* volume = kvs::StructuredVolumeObject::DownCast( BaseClass::volume() );
 ...
 */
/*==========================================================================*/
void ColoredMarchingCubes::calculate_normals_on_polygon(
    const Coords& coords,
 ...
 */
/*==========================================================================*/
void ColoredMarchingCubes::calculate_normals_on_vertex(
    const Coords& coords,
    const Connects& connections,
    Normals& normals )
{
...
// add
template <typename T>
void ColoredMarchingCubes::extract_colored_surfaces_with_duplication( const Volume* volume )
{
    // Variables wille store calculated the coordinate values and the normal vectors.
    Coords coords;
    Normals normals;
    ...
    // add
    m_cmap = BaseClass::transferFunction().colorMap();
    m_cmap.setRange( m_min_value, m_max_value );
        ...
        for ( kvs::UInt32 y = 0; y < ncells.y(); ++y )
        {
            for ( kvs::UInt32 x = 0; x < ncells.x(); ++x )
            {
                ...
                for ( size_t i = 0; Edge[ table_index ][i] != -1; i += 3 )
                {
                    ...
                    // add
                    const auto c01 = this->interpolate_color<T>( v0, v1 );
                    const auto c23 = this->interpolate_color<T>( v2, v3 );
                    const auto c45 = this->interpolate_color<T>( v4, v5 );
                    colors.push_back( c01.r() ); colors.push_back( c01.g() ); colors.push_back( c01.b() );
                    colors.push_back( c23.r() ); colors.push_back( c23.g() ); colors.push_back( c23.b() );
                    colors.push_back( c45.r() ); colors.push_back( c45.g() ); colors.push_back( c45.b() );
...
//        SuperClass::setColor( BaseClass::transferFunction().colorMap().at( m_isolevel ) );
        ...
        SuperClass::setNormals( kvs::ValueArray<kvs::Real32>( normals ) );
        SuperClass::setOpacity( 255 );
        SuperClass::setColorTypeToVertex(); // add
    }
}

// add
template <typename T>
void ColoredMarchingCubes::extract_colored_surfaces_without_duplication( const Volume* volume )
{
    const size_t volume_size = volume->numberOfNodes();
    const size_t byte_size   = sizeof( kvs::UInt32 ) * 3 * volume_size;
    kvs::UInt32* vertex_map = static_cast<kvs::UInt32*>( malloc( byte_size ) );
    ...
    Coords coords;
    Colors colors;
    this->calculate_isopoints<T>( vertex_map, coords, colors );
...
// add
template <typename T>
const kvs::RGBColor ColoredMarchingCubes::interpolate_color(
    const kvs::Vec3& vertex0,
    const kvs::Vec3& vertex1 ) const
{
    const T* const values = static_cast<const T*>( BaseClass::volume()->values().data() );
    ...
    const T* const reference = static_cast<const T*>( m_values.data() );
    const auto value0 = static_cast<kvs::Real64>( reference[ v0_index ] );
    const auto value1 = static_cast<kvs::Real64>( reference[ v1_index ] );
    const auto value = value0 + ratio * ( value1 - value0 );
    return m_cmap.at( value );
}
            ...
            for ( kvs::UInt32 x = 0; x < resolution.x(); ++x )
            {
                ...
                if ( x != ncells.x() )
                {
                         ...
                         ( static_cast<double>( values[id1] ) > isolevel ) )
                    {
                        ...
                        // add {
                        const auto isocolor = this->interpolate_color<T>( {x, y, z}, {x+1, y, z} );
                        colors.push_back( isocolor.r() );
                        colors.push_back( isocolor.g() );
                        colors.push_back( isocolor.b() );
                        // }
                ...
                if ( y != ncells.y() )
                {
                         ...
                         ( static_cast<double>( values[id2] ) > isolevel ) )
                    {
                        ...
                        // add {
                        const auto isocolor = this->interpolate_color<T>( {x, y, z}, {x, y+1, z} );
                        colors.push_back( isocolor.r() );
                        colors.push_back( isocolor.g() );
                        colors.push_back( isocolor.b() );
                        // }
                ...
                if ( z != ncells.z() )
                {
                         ...
                         ( static_cast<double>( values[id3] ) > isolevel ) )
                    {
                        ...
                        // add {
                        const auto isocolor = this->interpolate_color<T>( {x, y, z}, {x, y, z+1} );
                        colors.push_back( isocolor.r() );
                        colors.push_back( isocolor.g() );
                        colors.push_back( isocolor.b() );
                        // }
