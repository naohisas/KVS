...
function(kvs_cache_makefile_default name filename help)
    if(DEFINED CACHE{${name}})
        return()
    endif()

    if(DEFINED ENV{${name}})
        set(${name} "$ENV{${name}}" CACHE STRING "${help}")
        return()
    endif()
    ...
    foreach(_line IN LISTS _lines)
        ...
        if(_line MATCHES "^[ \t]*ifndef[ \t]+${name}([ \t]|$)")
            set(_ifndef_name "${name}")
        elseif(_ifndef_name AND _line MATCHES "^[ \t]*${name}[ \t]*:?=[ \t]*(.*)$")
            string(STRIP "${CMAKE_MATCH_1}" _default_value)
            set(${name} "${_default_value}" CACHE STRING "${help}")
            return()
